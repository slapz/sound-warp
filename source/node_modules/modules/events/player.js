    ...
    sw.on('player-ready', function() {
      ...
      function _handleTrackPlayback(track) {
        ...
        if (stream_url !== '') {
          ...
          sw.display.setArtwork(track.getArtworkUrl(Artwork.SIZE_300));
          sw.display.setBackgroundArtwork(track.getArtworkUrl(Artwork.SIZE_500));
          sw.display.getProgressBar().setMaxValue(track.getDuration() / 1000);
          sw.player.setVolume($('[data-action="sw.player.volume"]').val());
          sw.display.removeClass('now-playing', 'hidden');
      ...
      $('.playlist [data-_data-id]').bind('click', function(jqEvent) {
      ...
      _audio.on('loadeddata', function(jqEvent) {
        ...
        sw.display.getElement('sw.player.progress').removeAttr('disabled');
        sw.player.play();
      });
      ...
      _audio.on('ended', function(jqEvent) {
        jqEvent.preventDefault();
        controls.playToggle.find('.fa').removeClass('fa-pause').addClass('fa-play');
        sw.display.getElement('sw.player.progress').attr('disabled', true);
        sw.player.stop();
        _next(jqEvent);
        _playToggle(jqEvent);
      });
      ...
      _audio.on('timeupdate', function(jqEvent) {
        ...
        if (sw.player.ready()) {
          ...
          sw.display.setValue('sw.player.time.current', sw.display.timeString(sw.player.getCurrentTime()));
          sw.display.setValue('sw.player.time.duration', sw.display.timeString(sw.player.getDuration()));
        }
      });
