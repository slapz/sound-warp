...
module.exports = {
  register: function() {
    ...
    sw.on('loaded', function() {
      ...
      sw.view.setColumns({
        'Artwork': function(track) {
          let src = track.getArtworkUrl(Artwork.SIZE_67);
          if (src.length > 0) {
        ...
        'Duration': function(track) {
          //return sw.display.timeString(track.getDuration());
      ...
      sw.client.get(`/me/activities/tracks/affiliated`, function(activities) {
        ...
        if (tkeys.length > 0) {
          tkeys.forEach(function(index) {
            ...
            sw.view.data.set(track.get('id'), track);
          });
        }

        sw.display.setValue('sw.playlist.view', sw.view.render());
    ...
    sw.on('player-ready', function() {
      ...
      $('[data-track-id]').on('click', function(jqEvent) {
        jqEvent.preventDefault();

        sw.player.setSource(sw.client.url(sw.view.data.get($(this).attr('data-track-id')).get('stream_url')));
        sw.player.play();
      });
      ...
      _audio.on('play', function(jqEvent) {
        ...
        if (current) {
          ...
          $('[data-track-id="' + current.get('id') + '"]').addClass('now-playing');
        }
      });
